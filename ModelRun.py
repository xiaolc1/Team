...
import random
import dgl
from AMiner import graph, label
from AMiner.HAN import HAN

from AMiner.RGCNmodel import RGCN
from AMiner.feature_load import feature_skipgram, read_paper_features, read_author_features, feature_fusion, feat_test
from AMiner.HHGN import HHGN
from AMiner.simpleHGN import SimpleHGN
from AMiner.team import get_team_vectors, read_teams, sort_team_by_h_index, calculate_h_index_averages, \
...
plt.plot(team_h_indices_averages.numpy(), marker='o', linestyle='-', color='r', label='H-index Average')

# 设置标题和标签
plt.title('Team H-index Averages')
plt.xlabel('Team Index')
plt.ylabel('H-index Average')
plt.legend()
plt.grid(True)
